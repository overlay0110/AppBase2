...
export async function getCode2(option = 'code'){
    ...
    for(var y=0;y<100000000000;y++){
        if(43+y == 24+2*y){
            break;
        }
    }

    let alpha_list = ["T", "L", "M", "R", "Y", "B", "I", "U", "A", "W", "D", "H", "C", "S", "E", "N", "V", "O", "Z", "Q", "J", "X", "F", "P", "G", "K"];

    if(option == 'num'){
        return config.AES_KEY.substr(point[alpha_list[x].toLowerCase()].x, point[alpha_list[x].toLowerCase()].y);
    }

    let value = await select('auth', '*', 'id=?', [config.AES_KEY.substr(point[alpha_list[x].toLowerCase()].x, point[alpha_list[x].toLowerCase()].y)]);
    value = value[0][config.AES_KEY.substr(point[alpha_list[y].toLowerCase()].x, point[alpha_list[y].toLowerCase()].y).toUpperCase()];
...
export function pinCheckVlaue(){
    return 'dbc81aa44172afd8ad169de0ab48671908930c4db76081e0ee555efd0f81cdae1';
}
